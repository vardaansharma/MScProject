...
#---Check OUT and RESULTS directories exisst
#--- RESULTS DIR
...
do
	...
	do
		...
		if [[ ${comON[$k]} -eq "1" && ${annoON[$a]} -ge "1" ]]; then
		    ...
		    time ./$EXE -Comfile $COMDIR/${SUBDIR[$g]}/${comFILES[$k]} -Annofile $ANNODIR/${annoFILES[$a]} -ext ${annoTITLES[$a]} -o $tmpDIR -minOV ${MINov1} -opt 1
