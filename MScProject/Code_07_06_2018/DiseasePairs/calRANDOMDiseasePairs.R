...
loc <- read.table(sprintf("%s/%s/%s_%s.csv",subDIR[S],gdaDIR[gdas],subDIR[S],FILES[4]),sep="\t",header=T)

ds <- read.table(sprintf("%s/%s/%s_%s.csv",subDIR[S],gdaDIR[gdas],subDIR[S],FILES[1]),sep="\t",header=T)

RANds <- read.table(sprintf("RANDOM/%s/%s/random_%s.csv",subDIR[S],gdaDIR[gdas],FILES[1]),sep="\t",header=T)
...
sAB <- read.table(sprintf("%s/%s/%s_%s.csv",subDIR[S],gdaDIR[gdas],subDIR[S],FILES[2]),sep="\t",header=T)

#--- load random sAB values for each iteration
RAW_sAB  <- read.table(sprintf("RANDOM/%s/%s/RAW_random_sAB_%s.csv",subDIR[S],gdaDIR[gdas],FILES[3]),sep="\t",header=T)
...
CN <-  c("HDO.ID","Disease.long","Disease","N","HDO.ID","Disease.long","Disease","N","sAB","Separated","Overlap","zScore","pvalue","Separation/Overlap.than.chance","Bonferroni","q-value")
