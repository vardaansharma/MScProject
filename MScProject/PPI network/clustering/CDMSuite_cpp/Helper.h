...
class Helper {
 ...
 public:
  ...
  bool equals(double a, double b, double error_factor);
  int  rBinarySearch(vector<string> sortedVector, int first, int last, string key);
...
bool Helper::equals(double a, double b, double error_factor=1.0){
...
int Helper::rBinarySearch(vector<string> sortedVector, int first, int last, string key) {
   // function:
   ...
   if (first <= last) {
       ...
       if ( key.compare(sortedVector[mid]) == 0 ) 
           return mid;   // found it.
       else if (key.compare(sortedVector[mid].c_str()) < 0 ) 
