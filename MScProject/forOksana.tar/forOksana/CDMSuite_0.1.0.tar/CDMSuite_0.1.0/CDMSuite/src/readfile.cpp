...
readfile::readfile(network *NET, string *DATAIN, int nCOLS, int nROWS, int NUMERIC)
{ 
  int i,j,k,KK;
  ...
  this->NROWS     = nROWS;
  this->NCOLS     = nCOLS;
  this->isNumeric = NUMERIC;//are we dealing with alphaNumeric (0) or numeric (!0) ids
  this->OFFSET    = 0;
  this->Header    = false;
  this->Skip      = 0;
  this->dataset   = DATAIN;
  ...
  if( isNumeric == 0 ){
    ...
    for(k=0; k<KK; k++){
      ...
      //store two labels of edge
      if( j <= 1 )      
	labels.push_back( dataset[(i*NCOLS)+j] );
    ...
    for(k=0; k<KK; k++){
      ...
      if ( j <= 1 )
	labelsI.push_back( stoi(dataset[(i*NCOLS)+j]) );
